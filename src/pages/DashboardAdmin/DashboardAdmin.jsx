...
// 📌 Komponen Dashboard
const DashboardContent = ({ orders, handleStatusChange, getStatusColor, formatPrice, setShowBilling }) => {
  return (
    <div className="p-4 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 h-full">
      ...
      <div className="lg:col-span-2">
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-4 lg:p-6 h-full">
          <h3 className="text-lg lg:text-xl font-semibold text-gray-800 mb-4 lg:mb-6">Pesanan Terbaru</h3>
          <div className="space-y-3 lg:space-y-4">
            {orders.map((order) => (
              <div key={order.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 lg:p-4 hover:bg-gray-50 rounded-xl transition-colors">
                <div className="flex items-center space-x-3 lg:space-x-4 mb-2 sm:mb-0">
                  ...
                  <div>
                    <h4 className="font-semibold text-gray-800 text-sm lg:text-base">{order.name}</h4>
                    <p className="text-xs lg:text-sm text-gray-500">Pesanan {order.orderId}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-2 lg:space-x-4">
                  <button
                    onClick={() => handleStatusChange(order.id)}
                    className={`px-3 lg:px-4 py-1.5 lg:py-2 rounded-full text-xs lg:text-sm font-medium transition-colors ${getStatusColor(order.status)} cursor-pointer`}
                  >
                    {order.status}
                  </button>
                  <span className="font-bold text-blue-600 text-sm lg:text-base">{formatPrice(order.price)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-4 lg:p-6 text-center">
          <div className="relative w-24 h-24 lg:w-32 lg:h-32 mx-auto mb-4">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-lg lg:text-2xl font-bold text-gray-800">60%</span>
            </div>
          </div>
          <h4 className="font-semibold text-gray-800 text-sm lg:text-base">Penjualan Hari Ini</h4>
        ...
        <div className="p-4 lg:p-6 border-b border-gray-100">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center">
              <span className="text-white font-bold text-base lg:text-lg">CL</span>
            </div>
            <span className="text-lg lg:text-xl font-semibold text-gray-800">Dago ENG</span>
          </div>
        </div>
  ...
  <div className="p-4 lg:p-8 border-b border-gray-100">
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
      <div>
        ...
        <p className="text-gray-500 text-sm lg:text-base">Coffee Lab Creative Hub & Coffee Lab</p>
      </div>
      <div className="flex items-center space-x-2 lg:space-x-4">
